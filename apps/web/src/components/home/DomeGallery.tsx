...
// @ts-nocheck
// DomeGallery is vendored from React Bits (registry component, ~700 lines of
// imperative DOM work). It pre-dates our strict tsconfig (noUncheckedIndexedAccess)
// and uses lots of internal type-loose patterns. Keeping ts-nocheck here lets us
// drop the upstream code as-is and only audit the public surface (props).
'use client';
...
type DomeGalleryProps = {
  ...
  grayscale?: boolean;
  /** Called when a tile is opened (after the enlarge animation finishes). The
   *  href comes from the image item; consumer can navigate to it. */
  onImageOpen?: (info: { src: string; alt: string; href?: string }) => void;
};
...
function buildItems(pool: DomeImage[], seg: number) {
  ...
  const normalizedImages = pool.map((image) => {
    if (typeof image === 'string')
      return { src: image, alt: '', href: undefined as string | undefined };
...
export default function DomeGallery({
  ...
  const rotateBy = useCallback(
    (deltaX: number, deltaY = 0) => {
      ...
      const nextY = wrapAngleSigned(rotationRef.current.y + horizontalDelta / dragSensitivity);
      rotationRef.current = { x: rotationRef.current.x, y: nextY };
      applyTransform(rotationRef.current.x, nextY);
    },
    [applyTransform, dragSensitivity, stopInertia],
  );

  const onWheel = useCallback(
    (event: React.WheelEvent<HTMLElement>) => {
      event.preventDefault();
      rotateBy(event.deltaX, event.deltaY);
    },
    [rotateBy],
  );
  ...
  const triggerOpen = useCallback(
    (target: HTMLDivElement) => {
      if (onImageOpen) {
        const tile = target.closest('.item') as HTMLElement | null;
        const href = tile?.dataset.href || undefined;
        const src = tile?.dataset.src || '';
        const alt = tile?.dataset.alt || '';
        onImageOpen({ src, alt, href });
      } else {
        openItemFromElement(target);
      }
    },
    [openItemFromElement, onImageOpen],
  );
